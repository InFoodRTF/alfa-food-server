  ...
  def list(self, request, *args, **kwargs):
    ...
    if hasattr(user, 'parent'):
      ...
      if request.query_params:
        ...
        attendance_meal_category_name = request.query_params.get(
          'meal_category')
        attendance_date = request.query_params.get('date')
        if not (attendance_grade_name and attendance_meal_category_name
                and attendance_date):
          return Response("Один или несколько параметров не были переданы",
                          status=status.HTTP_400_BAD_REQUEST)

        attendance_grade = Grade.objects.get(name=attendance_grade_name)
        attendance_meal_category = MealCategory.objects.get(
          category_name=attendance_meal_category_name)
        ...
        if len(students_attendance) == 0 and attendance is None:
          return Response(
            "Отмечать нечего! Заказов на выбранные параметры ещё нет.",
